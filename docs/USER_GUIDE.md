...
深度搜索结合了 FTS5 全文搜索和向量语义搜索，提供更精准的检索结果。
...
| 场景 | 推荐设置 |
|------|----------|
| 快速查找精确关键词 | `/deep off`（FTS5 模式） |
| 语义理解、意图匹配 | `/deep on`（混合模式） |
| 需要理解上下文关联 | `/deep on`（混合模式） |

#### 工作原理

1. **自动记忆**：对话结束后自动存储关键信息
2. **智能检索**：根据当前上下文检索相关记忆
3. **上下文注入**：将相关记忆自动添加到对话上下文
