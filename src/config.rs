...
//!
//! Manages `~/.rupoo/config.toml` (user preferences) and
//! `~/.rupoo/credentials.toml` (API keys, chmod 600).
//!
//! Migrated from DB-only settings to file-based config for:
//! - Portability (copy ~/.rupoo/ to another machine)
...
/// Return the Rupoo data directory, defaulting to `~/.rupoo/`.
...
fn default_auto_approve_tools() -> Vec<String> {
    vec!["echo", "file_read", "list_directory", "run_tests", "web_search"]
        .into_iter().map(String::from).collect()
}
...
impl RupooConfig {
    /// Load config from `~/.rupoo/config.toml`, creating defaults if missing.
    ...
    /// Save config to `~/.rupoo/config.toml`.
...
impl CredentialsFile {
    /// Load credentials from `~/.rupoo/credentials.toml`.
    pub fn load() -> AgentResult<Self> {
        let path = data_dir().join("credentials.toml");
        if !path.exists() {
            return Ok(Self::default());
        }
        let content = std::fs::read_to_string(&path)
    ...
    /// Save credentials to `~/.rupoo/credentials.toml` with 0600 permissions.
    pub fn save(&self) -> AgentResult<()> {
        let dir = data_dir();
        std::fs::create_dir_all(&dir)
            .map_err(|e| AgentError::Config(format!("create config dir: {e}")))?;
...
pub fn init_config() -> AgentResult<PathBuf> {
    let dir = data_dir();
    std::fs::create_dir_all(&dir)
        .map_err(|e| AgentError::Config(format!("create ~/.rupoo: {e}")))?;
    ...
    for sub in &["db", "logs", "skills"] {
        ...
        if !sub_dir.exists() {
            std::fs::create_dir_all(&sub_dir)
                .map_err(|e| AgentError::Config(format!("create ~/.rupoo/{sub}: {e}")))?;
        }
    }
...
mod tests {
    ...
    fn test_safety_defaults() {
        ...
        assert!(safety.forbidden_commands.contains(&"sudo".to_string()));
        assert!(safety.auto_approve_tools.contains(&"file_read".to_string()));
    }
